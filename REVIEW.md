...
3. **Motion budget**: M6, M7, M8, L6, L7, L8. **Done** on `fix/review-batch-3-motion-budget`. AnsweringButton keeps both faces stacked in one grid cell, so its width never changes, and the face that takes over fades in on `--duration-fast` both ways, but not on first paint. The faces inherit Button's gap, so no size needs its own. The charts share `chartAnimation` (`chartMotion.ts`), which uses the exact `--ease-standard` curve rather than `ease-out`, with a test that fails if it drifts from the tokens in index.css. `motion.test.tsx` now fails on any literal time in motion.css. L8 went further than proposed, since the proposed selector missed three of sonner's movements: everything sonner moves (the stack, the toast, its content, a toast leaving from behind at 500 ms, and the swipe-out) is on `--duration-base`, with `!important` inside `no-preference`. With `--tw-ease` set globally, `arrive` dropped its own copy.
4. **Navigation state**: M2 and M3. Both need a decision on where per-view state (page, filters, tab) lives, and M2's page jump depends on it. **Done** on `fix/review-batch-4-navigation-state`. Decided: every view keeps its tab, page, search and filters while the app is open, and nothing is stored in the database. They live in `useViewState`, a `useState` whose value is copied into a `ViewStateProvider` held in App, above the view's error boundary. `useRequestedTab` is built on it and also remembers which menu request it already answered, so a view coming back does not answer the last one again. `useRememberedScroll` now ignores the scroll that answers a restore that fell short, so a clamp no longer overwrites the stored position. M2 took a different route from the fix above, on Fermín's choice: `addTransaction` resolves with the new id once the list has been read back, and TransactionsView owns the answer. It goes to the page holding the row, brings it into view with room for the toast (`scroll-my-24`), tints it, and shows the success toast itself. When the filters hide the row, that toast says so and offers "Limpiar filtros", which also switches to the row's currency, since the currency hides it just the same. `justWrittenTransaction` left the provider.
